...
def loss_function(prediction, true, time_temp):
    ...
    loss_1 = loss_mse + w * torch.sum(abs(prediction - true) * time_temp) / torch.numel(true)

    return scale * loss_1, loss_mse


class BiLstmMethod1(nn.Module):

    def __init__(self, n_features, hidden_dim=100, dropout=0.2, num_layers=2, output_dim=1):
        super(BiLstmMethod1, self).__init__()
...
if __name__ == '__main__':
    ...
    model = BiLstmMethod1(n_features=14,
